...
from fornitori_config import (
    FORNITORI,
    TEMI_REGIONALI,
    elenco_fornitori_per_ruolo_regione,
    match_fornitore,
)
from profilazione_locale import (
...
def riempi_slot_ricetta(template: dict, collezione_prodotti, indice_codici: dict, embedder,
    ...
    if is_tagliere:
        ...
        # 1. Salumi
        comp_salumi = get_comp_piano("salumi")
        if comp_salumi.get("quantita_target") is not None:
            target_salumi = int(comp_salumi.get("quantita_target"))
        ...
        if target_salumi > 0:
            if is_mare:
                sel_salumi = _seleziona_componente_tagliere(
                    ...
                    n_target=target_salumi, query_utente=comp_salumi.get("query_pulita", query_utente),
                    prodotti_esclusi=prodotti_esclusi,
            ...
            for idx, prod in enumerate(sel_salumi):
                slot_riempiti.append({
                    "ingrediente_richiesto": f"{etichetta} ({idx+1})",
        ...
        # 2. Formaggi
        comp_formaggi = get_comp_piano("formaggi")
        if comp_formaggi.get("quantita_target") is not None:
            target_formaggi = int(comp_formaggi.get("quantita_target"))
        ...
        abilita_formaggi = target_formaggi > 0 and (formaggi_di_default or vuole_formaggi_esplicito)
        ...
        if abilita_formaggi:
            sel_formaggi = _seleziona_componente_tagliere(
                ...
                n_target=target_formaggi, query_utente=comp_formaggi.get("query_pulita", query_utente),
                prodotti_esclusi=prodotti_esclusi,
