...
from fornitori_config import FORNITORI, elenco_fornitori_per_ruolo_regione
from profilazione_locale import rileva_canale_locale
from tassonomia_sofood import classifica_terra_mare
...
{{
  "intento": "tagliere_o_ricetta" oppure "ricerca_catalogo",
  "componenti": [
    {{
      "ruolo": "salumi" / "formaggi" / "mare" / "pane" / "olive" / "extra" / "ricerca",
      "quantita_target": intero (es. se chiede esplicitamente 4 prosciutti, scrivi 4) o null se non specificato,
      "sottocategoria_forzata": "prosciutto crudo" / "salame" o null se non esplicitata,
      "query_pulita": "LA TUA QUERY RIFORMULATA E PULITA (vedi regole sopra)",
      "esclusioni": ["cotto", "wurstel"]
    }}
  ],
  "filtri_metadati": {{
    "dieta": "vegano" / "vegetariano" / null
  }}
}}
"""
    import json
    ...
    for tentat in range(3):
        try:
            risposta_riscrittura = client_genai.models.generate_content(
                model=MODELLO_FALLBACK,  # Usa Flash Lite per non consumare quote
            ...
            testo_json = risposta_riscrittura.text.strip()
            if testo_json.startswith("```json"):
                testo_json = testo_json[7:-3].strip()
            piano_ricerca = json.loads(testo_json)
            
            # Estrai la query principale da passare come fallback o per la ricerca catalogo generica
            queries = []
            ...
            if ha_img and p_img not in testo_pulito:
                ...
                for riga in righe:
                    ...
                    if not inserito and riga.strip().startswith(("-", "*", "•")):
                        ...
                        # Logica più stringente: richiede che ALMENO 2 parole chiave corrispondano (o 1 se il nome ha solo 1 parola)
                        # e, se presente, che corrisponda anche il fornitore (se menzionato)
