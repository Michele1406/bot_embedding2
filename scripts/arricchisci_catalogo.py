...
from core.tassonomia_sofood import TASSONOMIA_LISTA, MAPPA_SOTTOCATEGORIA, SOTTOCATEGORIE_NOMI

# ============================================================================
# NOTA IMPORTANTE SUL FLUSSO DI CLASSIFICAZIONE (aggiornato)
# ============================================================================
# La classificazione tassonomica (reparto/categoria_tassonomia/sottocategoria,
# più i campi "Specifiche" e "Formato/Variante") NON viene più fatta da questo
# script chiamando Gemini prodotto per prodotto durante l'arricchimento. Ora
# passa da una pipeline offline dedicata che legge le schede tecniche (.txt)
# di ogni prodotto e produce Tassonomia.xlsx secondo lo standard ECR Grocery.
#
# Il flusso corretto oggi è:
#   1. Indicizzare in ChromaDB gli embedding dei prodotti NUOVI (questo file,
#      metodo indicizza_nuovi_prodotti) -> restano con tassonomia
#      "DA CLASSIFICARE" finché non passano dal punto 2.
#   2. Far girare la pipeline offline di classificazione ECR sulle schede
#      tecniche dei prodotti nuovi, per produrre/aggiornare Tassonomia.xlsx.
#   3. Applicare Tassonomia.xlsx ai metadati ChromaDB con
#      rigenera_tassonomia_db.py (nessuna chiamata API, puro aggiornamento
#      locale dei metadati già classificati nel punto 2).
#
# Il metodo classifica_tassonomia_batch() sotto è mantenuto SOLO come
# riferimento storico (classificazione LLM diretta contro la vecchia
# tassonomia libera a 73 voci): non è più il percorso consigliato e, di
# default, lo script NON lo esegue più (vedi __main__ in fondo al file).
# ============================================================================
...
class GestoreCatalogo:
            ...
            for cartella_prod in cartella_forn.iterdir():
                ...
                try:
                    ...
                    meta = {
                        ...
                        "sottocategoria": "DA CLASSIFICARE",
                        "categoria_tassonomia": "DA CLASSIFICARE",
                        "reparto": "DA CLASSIFICARE",
                    }
    ...
    def classifica_tassonomia_batch(self, batch_limit: int = 50):
        """[LEGACY] Assegna sottocategoria, categoria e reparto usando
        gemini-3.5-flash-lite contro la vecchia tassonomia libera a 73 voci.

        NON è più il percorso consigliato per classificare il catalogo: da
        quando la classificazione passa dalla pipeline offline standard ECR
        (Tassonomia.xlsx + rigenera_tassonomia_db.py), questo metodo assegna
        categorie con un vocabolario diverso da quello ufficiale e andrebbe a
        introdurre incoerenze. Mantenuto solo per compatibilità/riferimento
        storico: non viene più richiamato di default da __main__."""
        print("\n" + "="*70)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Indicizzazione catalogo So Food (embedding). La classificazione tassonomica ora passa da Tassonomia.xlsx + rigenera_tassonomia_db.py, vedi nota a inizio file.")
    ...
    parser.add_argument("--classificazione-legacy", action="store_true",
                         help="Esegue anche il vecchio step di classificazione LLM (batch-size configurabile con --batch-size). SCONSIGLIATO: usa una tassonomia diversa da quella ufficiale ECR. Preferisci far girare la pipeline offline ECR e poi rigenera_tassonomia_db.py.")
    parser.add_argument("--batch-size", type=int, default=50, help="Solo con --classificazione-legacy: numero di prodotti da classificare in questo blocco")
    args = parser.parse_args()

    gestore = GestoreCatalogo()
    # 1. Carica i prodotti mancanti dei nuovi fornitori (restano "DA CLASSIFICARE"
    #    finché non passano dalla pipeline ECR + rigenera_tassonomia_db.py)
    gestore.indicizza_nuovi_prodotti(cartelle_target=args.target_fornitori, max_nuovi=args.nuovi_max)

    if args.classificazione_legacy:
        print("[AVVISO] Stai usando lo step di classificazione LEGACY (tassonomia libera, non ECR). Consigliato solo per test.")
        gestore.classifica_tassonomia_batch(batch_limit=args.batch_size)
    else:
        print("[INFO] Nuovi prodotti indicizzati con tassonomia 'DA CLASSIFICARE'.")
        print("[INFO] Prossimo passo: far girare la pipeline offline di classificazione ECR sulle loro schede tecniche,")
        print("       aggiornare Tassonomia.xlsx, poi lanciare: python rigenera_tassonomia_db.py")
